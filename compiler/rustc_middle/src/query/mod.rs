...
rustc_queries! {
    ...
    /// Given an `fn_def_id`, return the list of rpits returned by the function.
    query get_fn_rpits(fn_def_id: DefId) -> &'tcx [DefId] {
        desc { |tcx| "computing rpits returned by `{}`", tcx.def_path_str(fn_def_id) }
