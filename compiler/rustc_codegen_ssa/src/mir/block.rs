...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    // Already removed most of instrinsics specific and I may have already the instance
    // I can get the instance from the def_id (given that is a lang item I know it)
    // Instance::Kind -> Item(DefId) we know that is true, we know it has a single argument, that
    // argument is &T.
    // There's a simplified path through this function to avoid known things.
    // We don't have a terminator either, so we can remove that.
    fn codegen_lang_fn_call(
        &mut self,
        helper: TerminatorCodegenHelper<'tcx>,
        bx: &mut Bx,
        terminator: &mir::Terminator<'tcx>,
