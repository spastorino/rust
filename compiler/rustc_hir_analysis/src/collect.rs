...
fn assoc_items_for_rpits(tcx: TyCtxt<'_>, trait_fn_def_id: DefId) -> &'_ [DefId] {
    struct RPITVisitor {
        rpits: Vec<DefId>,
    }
...
fn rpitit_associated_item(tcx: TyCtxt<'_>, opaque_ty_def_id: LocalDefId) -> Option<LocalDefId> {
    ...
    if let Some(trait_def_id) = tcx.opt_parent(fn_def_id) && tcx.def_kind(trait_def_id) == DefKind::Trait {
        ...
        let local_def_id = impl_trait_assoc_ty.def_id();
        let def_id = local_def_id.to_def_id();

        impl_trait_assoc_ty.opt_def_kind(Some(DefKind::AssocTy));
        ...
        impl_trait_assoc_ty.opt_local_def_id_to_hir_id(None);

        impl_trait_assoc_ty.def_ident_span(Some(span));

        // FIXME should probably be Restricted to the crate?
        impl_trait_assoc_ty.visibility(Visibility::Public);

        impl_trait_assoc_ty.generics_of(tcx.generics_of(opaque_ty_def_id).clone());

        impl_trait_assoc_ty.explicit_predicates_of(ty::GenericPredicates {
            parent: Some(trait_def_id),
            predicates: &[],
        });

        impl_trait_assoc_ty.inferred_outlives_of(&[]);

        // FIXME we probably want the following:
        // impl_trait_assoc_ty.impl_defaultness(tcx.impl_defaultness());
        impl_trait_assoc_ty.impl_defaultness(Defaultness::Final);

        impl_trait_assoc_ty
            .type_of(tcx.mk_opaque(def_id, InternalSubsts::identity_for_item(tcx, def_id)));

        impl_trait_assoc_ty.explicit_item_bounds(&[]);
