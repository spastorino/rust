...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    ...
    fn lower_res(&mut self, res: Res<NodeId>) -> Res {
        let res = res.map_def_id(|def_id| {
            if let Some(local_def_id) = def_id.as_local() {
                self.get_remapped_def_id(local_def_id).to_def_id()
            } else {
                def_id
            }
        });
                        ...
                        hir::GenericArg::Type(self.arena.alloc(hir::Ty {
                            ...
                            kind: hir::TyKind::Path(hir::QPath::Resolved(
                                    ...
                                    self.arena.alloc(hir::Path {
                                        res,
                                        span,
                                        segments: arena_vec![self; hir::PathSegment::new(self.lower_ident(param.ident), self.next_id(), res)],
                                    }),
                            )),
                        }))
                    },
                                    ...
                                    hir::Expr {
                                        ...
                                        kind: hir::ExprKind::Path(hir::QPath::Resolved(
                                            ...
                                            this.arena.alloc(hir::Path {
                                                res,
                                                span,
                                                segments: arena_vec![this; hir::PathSegment::new(this.lower_ident(param.ident), this.next_id(), res)],
                                            }),
                                        )),
                                        span,
                                    },
    ...
    fn lower_lifetime_binder<R>(
        ...
        if !self.generics_def_id_map.is_empty() {
