...
struct LoweringContext<'a, 'hir: 'a> {
    ...
    check_in_scope_lifetime_bounds: bool,
    ...
    remapped_def_id: FxHashMap<DefId, DefId>,
...
pub fn lower_crate<'a, 'hir>(
    ...
    LoweringContext {
        ...
        remapped_def_id: FxHashMap::default(),
        generator_kind: None,
        task_context: None,
        current_item: None,
        ...
        check_in_scope_lifetime_bounds: false,
        in_scope_lifetime_bounds: Vec::new(),
        in_scope_generic_params: Vec::new(),
        in_scope_where_clause: None,
...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    ...
    fn with_in_scope_lifetime_bounds<R>(&mut self, f: impl FnOnce(&mut Self) -> R) -> R {
        self.check_in_scope_lifetime_bounds = true;
        ...
        self.check_in_scope_lifetime_bounds = false;
        result
    }
    ...
    fn with_in_scope_generic_defs<T>(
        ...
        let old_lt_len = self.in_scope_lifetimes.len();
        let old_gen_len = self.in_scope_generic_params.len();

        let mut lt_def_names = Vec::new();
        ...
        self.in_scope_lifetimes.extend(lt_def_names);
        self.in_scope_generic_params.extend(generic_def_names);

        let res = f(self);

        self.in_scope_lifetimes.truncate(old_lt_len);
        self.in_scope_generic_params.truncate(old_gen_len);
        res
    }
        ...
        let (lifetimes_to_define, (mut lowered_generics, impl_trait_defs, res)) = self
            .collect_in_band_defs(parent_def_id, |this| {
                this.with_anonymous_lifetime_mode(anonymous_lifetime_mode, |this| {
                    // TODO should call with_in_scope_generic_defs in all the places where we have
                    // impl traits in return position and only when there are impl traits in return
                    // position
                    this.with_in_scope_generic_defs(&generics.params, |this| {
                        ...
                        let hir_generics = this.lower_generics_mut(
                            generics,
                        ...
                        let current_where_clause = std::mem::take(&mut this.in_scope_where_clause);
                        this.in_scope_where_clause = Some(generics.where_clause.clone());
                        let res = f(this, &mut impl_trait_defs);
                        this.in_scope_where_clause = current_where_clause;
                        (hir_generics, impl_trait_defs, res)
                    })
                })
            });
    ...
    fn lower_ty_direct(&mut self, t: &Ty, mut itctx: ImplTraitContext<'_, 'hir>) -> hir::Ty<'hir> {
        let kind = match t.kind {
            ...
            TyKind::ImplTrait(def_node_id, ref bounds) => {
                let span = t.span;
                match itctx {
                    ImplTraitContext::ReturnPositionOpaqueTy { fn_def_id, origin } => {
                    ...
                    for hir_bound in hir_bounds {
                        if let hir::GenericBound::Outlives(l) = hir_bound {
                            let name = match l.name {
                                hir::LifetimeName::Underscore => hir::ParamName::Plain(
                                    Ident::with_dummy_span(kw::UnderscoreLifetime),
                                ),
                                hir::LifetimeName::Param(param_name) => {
                                    if let ParamName::Plain(ident) = param_name {
                                        lctx.in_scope_lifetime_bounds.push(ident);
                                    }
                                    param_name
                                }
                                _ => panic!("expected `LifetimeName::Param` or `ParamName::Plain`"),
    ...
    fn lower_lifetime(&mut self, l: &Lifetime) -> hir::Lifetime {
        ...
        match l.ident {
            ...
            ident => {
                if self.check_in_scope_lifetime_bounds
                    && self
                        .in_scope_lifetime_bounds
                        .iter()
                        .find(|&lifetime| *lifetime == ident)
                        .is_none()
                {
                    self.new_named_lifetime(l.id, span, hir::LifetimeName::Static)
                } else {
