...
fn type_check_internal<'a, 'gcx, 'tcx, F>(
    ...
    mut extra: F,
    errors_buffer: Option<&mut Vec<DiagnosticBuilder<'a>>>,
)
...
impl<'a, 'gcx, 'tcx> TypeChecker<'a, 'gcx, 'tcx> {
    ...
    fn typeck_mir<'cx>(&mut self,
                       mir: &Mir<'tcx>,
                       mut errors_buffer: Option<&mut Vec<DiagnosticBuilder<'cx>>>)
        where 'cx: 'a, 'gcx: 'cx
    {
    ...
    fn run_pass<'a, 'tcx>(&self, tcx: TyCtxt<'a, 'tcx, 'tcx>, src: MirSource, mir: &mut Mir<'tcx>) {
        ...
        tcx.infer_ctxt().enter(|infcx| {
            let _ = type_check_internal(
                &infcx, def_id, param_env, mir, &[], None, None, |_| (), None);
