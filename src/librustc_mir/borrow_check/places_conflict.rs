...
use rustc::mir::{NeoPlace, PlaceBase, ProjectionElem};
use rustc::mir::PlaceElem;
...
pub(super) fn borrow_conflicts_with_place<'gcx, 'tcx>(
    ...
    let borrow_place = &tcx.as_new_place(borrow_place);
    let access_place = &tcx.as_new_place(access_place);
    debug!(
        "borrow_conflicts_with_place({:?}, {:?}, {:?}, {:?})",
    ...
    match place_base_conflict(tcx, &borrow_place.base, &access_place.base) {
        ...
        Overlap::EqualOrDisjoint => place_elems_conflict(
            tcx,
            mir,
            &borrow_place,
            &access_place,
...
fn place_elems_conflict<'gcx, 'tcx>(
    tcx: TyCtxt<'_, 'gcx, 'tcx>,
    mir: &Mir<'tcx>,
    borrow_place: &NeoPlace<'tcx>,
    access_place: &NeoPlace<'tcx>,
    borrow_kind: BorrowKind,
    access: AccessDepth,
    bias: PlaceConflictBias,
) -> bool {
    ...
    let mut borrow_elems = borrow_place.elems.iter();
    let mut access_elems = access_place.elems.iter();
    let mut borrow_base_ty = borrow_place.base.ty(mir);
    loop {
        // loop invariant: borrow_c is always either equal to access_c or disjoint from it.
        if let Some(borrow_elem) = borrow_elems.next() {
            debug!(
                "borrow_conflicts_with_place: borrow_elem = {:?}",
                borrow_elem
            );

            if let Some(access_elem) = access_elems.next() {
                debug!(
                    "borrow_conflicts_with_place: access_elem = {:?}",
                    access_elem
                );

                // Borrow and access path both have more elements.
                //
                // Examples:
                //
                ...
                //
                // Here we only see the elements we have checked so
                // far (in our examples, just the first element). We
                // check whether the elements being borrowed vs
                // accessed are disjoint (as in the second example,
                // but not the first).
                match place_element_conflict(borrow_elem, access_elem, bias, borrow_base_ty) {
                    Overlap::Arbitrary => {
            ...
            if borrow_kind == BorrowKind::Shallow && access_elems.next().is_some() {
...
fn place_element_conflict<'a, 'gcx: 'tcx, 'tcx>(
    ...
    elem2: &PlaceElem<'tcx>,
    bias: PlaceConflictBias,
    borrow_base_ty: rustc::ty::Ty<'tcx>,
) -> Overlap {
    match (elem1, elem2) {
